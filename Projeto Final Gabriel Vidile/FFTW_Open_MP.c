...
int main(int argc, char *argv[])
{
    ...
    // Alocar arrays para o vetor de entrada
    double *xr = (double *)malloc(N * sizeof(double));
    double *xi = (double *)malloc(N * sizeof(double));
    fillInput(xr, xi, N);

    // Para propósitos de verificação
    ...
    // Alocar arrays para o vetor de saída
...
#pragma omp parallel for
    for (int k = 0; k < N; k++)
    {
        ...
        {
            // Parte real de X[k]
            tmpXr += xr[n] * cos(n * k * PI2 / N) + idft * xi[n] * sin(n * k * PI2 / N);
            // Parte imaginária de X[k]
            tmpXi += -idft * xr[n] * sin(n * k * PI2 / N) + xi[n] * cos(n * k * PI2 / N);
        }
    ...
    // Normalizar se estiver fazendo IDFT
    if (idft == -1)
    {
#pragma omp parallel for
        for (int n = 0; n < N; n++)
